...
> = ({ register, setValue, errors, control }) => {
  ...
  const [selectedDob, setSelectedDob] = useState<string>(formDob);

  // Update state when form value changes (for edit mode or when navigating steps)
  useEffect(() => {
    setSelectedDob(formDob);
  }, [formDob]);
